...
<#
.SYNOPSIS
    Gear Lathe Feeder Server - Interactive Setup / Configuration Wizard (Windows)

.DESCRIPTION
    Configures the Gear Lathe Feeder Server by writing appsettings.Production.json,
    then optionally installs and starts the GearLatheFeeder.Server Windows Service.

    Automatically copies deployment files to C:\GearLatheFeeder (or custom -InstallDir).
    If GearLatheFeeder.Server is running, it will be stopped before copy and restarted after.
...
.EXAMPLE
    # From the dist/ folder as Administrator:
    powershell -ExecutionPolicy Bypass -File setup.ps1

.EXAMPLE
    # Install to a custom directory:
    powershell -ExecutionPolicy Bypass -File setup.ps1 -InstallDir D:\GearLatheFeeder

.EXAMPLE
    # Re-run just to update config:
    powershell -ExecutionPolicy Bypass -File setup.ps1
#>
...
$Host.UI.RawUI.WindowTitle = 'Gear Lathe Feeder Server Setup'

# --- Helper functions ---------------------------------------------------------
function Write-Banner {
    Write-Host ''
    Write-Host '================================================================' -ForegroundColor Cyan
    Write-Host '|     Gear Lathe Feeder Server - Setup and Configuration       |' -ForegroundColor Cyan
    Write-Host '================================================================' -ForegroundColor Cyan
    Write-Host ''
}
...
if (Test-Path $AppSettings) {
    try {
        ...
        $cfg = @{
            ...
            StoragePath  = if ($existing.FileStorage) { $existing.FileStorage.RootPath }  else { $null }
            RemotePath   = if ($existing.FileStorage) { $existing.FileStorage.RemotePath } else { $null }
        }
    } catch {
...
# Prefer pre-robocopy values if available (prevents overwritten defaults from being used)
...
do {
    ...
    try {
        ...
        if ($procName -eq 'Server.Api') {
            Write-Warn "Port $apiPort is used by GearLatheFeeder.Server (will be restarted). OK."
        } else {
            $portInUse = $true
...
Write-Section 'File Storage'
$storagePath = Read-Input 'File Storage Root Path' $def_storage
$remoteStoragePath = Read-Input 'File Storage Remote Path (optional)' $def_remotePath
...
$production = [ordered]@{
    ...
    FileStorage = [ordered]@{
        ...
        RemotePath = if ([string]::IsNullOrWhiteSpace($remoteStoragePath)) { $null } else { $remoteStoragePath.Trim() }
    }
    AllowedHosts = '*'
}
...
# Create storage folder if it's relative to server dir
$absStorage = if ([System.IO.Path]::IsPathRooted($storagePath)) {
    $storagePath
} else {
    Join-Path $ServerDir $storagePath
}
if (-not (Test-Path $absStorage)) {
    New-Item -ItemType Directory -Force -Path $absStorage | Out-Null
    Write-OK "Created storage folder: $absStorage"
}

$trimmedRemoteStorage = if ([string]::IsNullOrWhiteSpace($remoteStoragePath)) { $null } else { $remoteStoragePath.Trim() }
if ($trimmedRemoteStorage) {
    $absRemoteStorage = if ([System.IO.Path]::IsPathRooted($trimmedRemoteStorage)) {
        $trimmedRemoteStorage
    } else {
        Join-Path $ServerDir $trimmedRemoteStorage
    }

    if (-not (Test-Path $absRemoteStorage)) {
        New-Item -ItemType Directory -Force -Path $absRemoteStorage | Out-Null
        Write-OK "Created remote storage folder: $absRemoteStorage"
    }
}
...
if ($null -eq $existingService) {
    ...
    # Always update binPath on existing service (critical fix for reinstalls to new -InstallDir or updated build)
    # This ensures the service points to the correct executable after robocopy.
...
Write-Host "  Storage path  : $absStorage"               -ForegroundColor Gray
Write-Host ''
